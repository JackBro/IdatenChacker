...
Enemy::Enemy(){
	...
	SEDAMAGE.lpstrElementName = L"res/SE/damage.wav";
	SETAKED.lpstrElementName = L"res/SE/pop.wav";
...
int Enemy::Enemy_paint(HDC hdc){
	...
	enemyhdc = hdc;
	//DebugStringVal("%d", enemy.onActive, hdc, enemy.x, enemy.y, 20);
	...
	if (enemy.onActive & 1 && enemy.onActive > 0){
		...
		if (ex <= pw && px <= ew && ey <= ph && py <= eh){
			if (plstats->c_flg != 3 && plstats->c_flg != 2){
				...
				mciSendCommand(NULL, MCI_CLOSE, 0, (DWORD)&SEDAMAGE);
				mciSendCommand(NULL, MCI_CLOSE, 0, (DWORD)&SETAKED);


				if (ey <= ph - 12){
					DEADflg = -1;//??キャラに影響する 死亡の扱いは不明。
					mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_ELEMENT, (DWORD)&SEDAMAGE);
					...
					plstats->c_flg = 4;
					mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_ELEMENT, (DWORD)&SETAKED);
				

					mciSendCommand(SETAKED.wDeviceID, MCI_PLAY, 0, (DWORD)&SEENEMY);
				}
			}
