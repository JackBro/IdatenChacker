...
void Ranking_save(HDC hdc)
{
	...
	if ((fp = fopen(FileName, "w")) == NULL)
	{
		...
		if (r_time[0] > S_time){		//ランキング1位より現在プレイしてる人のほうが早ければ
			...
			fprintf(fp, "1位 %f秒\n",r_time[0]);
			DebugStringColor("1位 %.1f秒", r_time[0],	hdc, 480, 65 * (2 + 3), 30, 0, 0, 0);

			fprintf(fp, "2位 %f秒\n",r_time[1]);
			DebugStringColor("2位 %.1f秒", r_time[1], hdc, 480, 65 * (2 + 4), 30, 0, 0, 0);

			fprintf(fp, "3位 %f秒\n", r_time[2]);
			DebugStringColor("3位 %.1f秒", r_time[2], hdc, 480, 65 * (2 + 5), 30, 0, 0, 0);
