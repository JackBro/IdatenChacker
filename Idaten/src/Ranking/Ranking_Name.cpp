...
bool Ranking_Name::Reading()
{
	...
	if (ifs.fail()) {			//ランキングファイルが無かったら
		...
		for (i = 0; i < 5; i++){
			rank.push_back(std::make_pair("あいうえお", 999.0 - i));
		}
	}
	else{
...
int Ranking_Name::NameInput(HWND hWnd2)
{
	...
	MessageBox(hWnd2, name, TEXT("名前"), MB_OK);
...
bool Ranking_Name::Ranking_Paint(HDC hdc){
	...
	for (i = 0; i < 5; i++){
		...
		MultiByteToWideChar(CP_OEMCP, MB_PRECOMPOSED, str, -1, buf[i], sizeof(buf));	//charからTCHAR

		ofs << rank[i].first << "," << rank[i].second << "," << std::endl;		//テキストファイルに書き込み

		DebugStringColor("%d位", i + 1, hdc, 300, 65 * (4.5 + i), 27, 0, 0, 0);			//描画
		DebugStringColor("%.1lf秒", rank[i].second, hdc, 650, 65 * (4.5 + i), 27, 0, 0, 0);			//描画
		DebugStringColor("名前 %s", buf[i], hdc, 350, 65 * (4.5 + i), 27, 0, 0, 0);							//描画
	}
		return true;
}
